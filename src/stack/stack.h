...
template<typename T>
stack<T>::stack(stack<T> const& outer) try: stack() { 
	...
	while ( curr ) {
		...
		tracer->next_ = new node(next->data_);
		tracer = tracer->next_;
		curr = curr->next_;
	}
...
template<typename T>
stack<T>::stack(stack<T>&& outer): stack() {
	swap(*this, outer);
}
...
template <typename T>
template <typename U>
void stack<T>::push(U&& value) { push_front( std::forward<U>(value) ); }
