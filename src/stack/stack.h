...
template <typename T> class stack final {
  ...
  public:
    ...
    explicit node(T &&value, node *next = nullptr)
        : data_(std::move(value)), next_(next){};
...
// start stack
template <typename T> stack<T>::stack() : head_(nullptr) {}

template <typename T> stack<T>::stack(stack<T> const &outer) try : stack() {
...
template <typename T> stack<T>::stack(stack<T> &&outer) : stack() {
  swap(*this, outer);
}
...
template <typename T> bool stack<T>::empty() const { return head_ == nullptr; }

template <typename T>
template <typename U>
void stack<T>::push_front(U &&value) try {
...
template <typename T> template <typename U> void stack<T>::push(U &&value) {
...
template <typename T> void stack<T>::pop() { pop_front(); }

template <typename T> std::optional<T> stack<T>::peep() const {
